...
### Clarifying finding — phase 04 (help): a harness-only wrapper component's chunk survives the real build

Every feature phase before this one (03, and this map's own template for it) mounts the feature's real, already-shipping top-level component (`Profile.svelte`) in its harness — that component's JS chunk always ships in the real build anyway, since the feature's real page also imports it. Help has no single top-level component: `HelpView.svelte` and `HelpSearch.svelte` are two independently-mounted islands Terminal.svelte composes at runtime (deciding when `?` opens the palette vs. routes a key to the view), so its harness needed a new, harness-only wrapper (`src/features/help/tests/ui/harness/HelpHarness.svelte`) to reproduce just enough of that composition for both pieces to be reachable from one page.

Astro/Vite's static build emits a JS chunk for every module reachable from a page's *import graph*, independent of whether `getStaticPaths` actually renders a route for it — `[feature].astro`'s `import HelpHarness from "..."` is a static, unconditional import, so `pnpm build` (real, `PORTFOLIO_FIXTURES` unset) still emits `dist/_astro/HelpHarness.<hash>.js` (998 bytes), even though `getStaticPaths` returns no `"help"` entry for that build and no HTML page ever references it. Verified empirically: `grep -rl "HelpHarness" dist --include="*.html"` is empty after `pnpm build` (no page links to it) and finds `dist/harness/help/index.html` after `pnpm build:fixtures` (the only build where it's actually referenced) — the chunk is unreachable dead weight in a real build, not a functioning harness page. D24(a)'s contract ("zero harness **pages** may exist in a real build") holds on its own terms — no `dist/harness/` route output exists in a real build, matching phase 03's `find dist -iname "*harness*"` check in spirit — but that exact check no longer returns fully empty once a feature needs a harness-only component (it now also matches this dead chunk's filename). This wasn't eliminable in scope: client directives require statically-analyzable imports (no dynamic-import gating available without editing `astro.config.mjs`'s `vite.define`, out-of-context wiring; a second harness route per feature would re-litigate D24's settled single-route mechanism). Recorded here as a clarifying finding for phases 05–11: any later feature whose harness needs a purpose-built wrapper (rather than reusing a real, already-shipping component) will hit the same dead-chunk pattern — this is inherent to the mechanism, not a phase-04-specific bug, and needs an orchestrator ruling on whether to bless it as-is or track an env-gated-island cleanup for the post-migration hardening pass.
