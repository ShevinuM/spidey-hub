...
> **Status: closed 2026-09-12.** Binding decisions: `Instructions/00-phases.md` (the only cross-phase document this plan may read). This plan is self-contained: every mechanism it needs is stated here verbatim, so the executor never reads another phase's folder.
...
- **R007** — two essays duplicated verbatim three times each (`NotificationBell.svelte`, `NotificationsPanel.svelte`, `ToastStack.svelte`). Both now have their full text in exactly one home, `NotificationsPanel.svelte` — chosen on the `:hover` count, which is decisive: **six** `:hover` pairs versus one each in `NotificationBell.svelte` and `ToastStack.svelte`. (**Correction, per the closing audit:** an earlier version of this line also claimed two `-global-` keyframes "versus one elsewhere." That half is wrong — `ToastStack.svelte` also has two (`-global-toastIn`, `-global-strand`), so keyframe count is a tie and justifies nothing. The `:hover` margin alone carries the choice.) The other two components now carry a single-line pointer naming `NotificationsPanel.svelte` in place of the full text.
