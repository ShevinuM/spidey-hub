...
test("computeThroughputMbps: sums transferSize>2000 && duration>1 entries, floors at 0.2 Mb/s, null when none qualify", () => {
  ...
  expect(mbps !== null).toBeTruthy();
  expect(Math.abs((mbps as number) - 8) < 1e-9).toBeTruthy();
...
test("netStats: prefers navigator.connection's own downlink/rtt; falls back to measured/smoothed/defaults", () => {
  ...
  expect(fallback.down).toBe(30);
  expect(fallback.rtt).toBe(80);
  expect(fallback.type).toBe("FAST"); // down > 20

  // Nothing measured yet either: hardcoded defaults (5 Mb/s / 60ms / "LINK").
  expect(netStats(undefined, null, null)).toEqual({ down: 5, rtt: 60, type: "LINK" });
...
test("smoothRtt: 0.6/0.4 blend, rounded; first sample passes through as-is", () => {
  expect(smoothRtt(null, 123.4)).toBe(123);
  expect(smoothRtt(100, 200)).toBe(Math.round(100 * 0.6 + 200 * 0.4));
});
...
test("hueForHeight: 212 at h=0 (blue), 0 at h=1 (red)", () => {
  expect(hueForHeight(0)).toBe(212);
  expect(Math.abs(hueForHeight(1) - 0) < 1e-9).toBeTruthy();
});
