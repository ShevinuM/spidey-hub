...
// API — several assertions below are pinned to that checked-in data;
// re-verify them if `pnpm generate` is ever re-run against a different
// upstream state.
...
test.describe("Builds: commit click loads a tree (no popup); 'o' opens GitHub", () => {
  test("clicking a commit fetches its tree into panel [2] — no new tab", async ({ page, context }) => {
    ...
    await expect(page.locator('[data-testid="builds-tree-row"][data-entry-name="README.md"]')).toBeVisible();
    await expect(page.locator('[data-testid="builds-panel-2"]')).toContainText("@e3d098ef");
...
test.describe("Builds: client-side commit refresh (panel [3] selection only)", () => {
  // Default-highlighted repo on load (idx 0) is daily-tech-digest; its
  // committed snapshot head is "e3d098ef" (src/generated/commits/
  // daily-tech-digest.json — re-verify if `pnpm generate` is re-run).
  ...
  test("route-abort keeps the committed snapshot list intact", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="builds-commit-row"]').first()).toHaveAttribute("data-sha8", "e3d098ef");
  });
