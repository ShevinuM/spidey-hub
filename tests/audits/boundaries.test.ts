...
// reports a clean tree while those edges keep crossing. A text scan sees every
// file type and every specifier verbatim.
//
...
test("each boundary rule fires against a synthetic violation, and a stale entry is reported", () => {
  ...
  // None of the five is allowlisted, so all five must be reported as unlisted.
  expect(diffAgainstAllowlist(detected, ALLOWLIST).unlisted).toHaveLength(5);
