...
GitHub API; a yazi-style, depth-generic Employment Records browser; a vim-lite
...
(every `src/content/repositories/*.md` file, browsable as its own tiny repo —
...
1. Add the upstream repo as a git submodule under `repos/`:
   ...
   ```
2. Add a content file `src/content/repositories/<repo-name>.md` with
   frontmatter matching the `repositories` collection schema
...
1. Delete `src/content/repositories/<name>.md`.
...
`src/content/repositories/*.md` for the real site
...
exactly match a fresh index built from `fixtures/repositories/*.md` (same
...
editing a `fixtures/repositories/*.md` file (or adding/removing one) without
...
**To regenerate it** after touching `fixtures/repositories/*.md`, run this
...
node -e '
  ...
  const dir = "fixtures/repositories";
...
Employment Records' tree is **depth-generic** (PLAN.md Iteration 3 Phase 1 —
`EmploymentRecords.svelte` derives its levels from each file's own content path,
...
1. Add `src/content/personnel/<company-slug>/<role-slug>/role.md` (add an
   ...
   - The company (top-level directory) list has no separate order file —
     `EmploymentRecords.svelte` lists root-level directories alphabetically by
   ...
   - `employmentType` should match the role's containing directory name
     ...
     `EmploymentRecords.svelte`, independent of any directory structure the loader
...
Employment Records, the vim engine, the boot sequence, the cmdline box, real panes/
...
| key | action |
|---|---|
| `Ctrl-b` then `1`-`5` / `0` | jump straight to a window — Builds / Employment Records / Retina-V / Profile / Help / dashboard; there are no bare-key shortcuts (see "tmux prefix" below) |
...
| `:` | open the floating Cmdline box (from any view, and inside an open file editor as its ex-command line) — a literal `:` still types normally inside the grep query, the Employment Records filter, the rename prompt, and a focused shell |
...
| action | targets |
|---|---|
| click a status-bar window | jump straight to it — `0:dashboard 1:builds 2:employment 3:retina-v 4:profile 5:help` (the number/route stay fixed even after a window auto-renames — see "Shell builtins" below) |
...
| prefixed key | action |
|---|---|
| `1` / `2` / `3` / `4` / `5` | jump straight to Builds / Employment Records / Retina-V / Profile / Help |
| `0` | dashboard |
| `n` | next window (dashboard → builds → employment → retina-v → profile → help → dashboard …) |
...
| `[` | **copy-mode**: opens a generic overlay over the active pane's text (editor buffer, the focused Builds panel, the Employment Records row list, the Profile summary, the tracker HUD, the Help content, a shell's scrollback, or the dashboard menu) — vim-navigate it, `v` to start a selection, `y`/`Enter` to yank and exit, `q`/`Esc` to exit without yanking |
| `]` | **paste-buffer**: inserts the last copy-mode/vim yank into the active text input (grep query, Employment Records filter, the rename prompt, a shell's input line, or the editor's in-buffer search) — a status-line message appears if nothing's active or nothing's been yanked yet |
...
| `view-names` | list the launchable programs — `dashboard`, `builds`, `employment`, `retina-v`, `profile`, `help` |
...
Builds' and Employment Records' file viewers identically — NORMAL / VISUAL /
...
| `h` | up one level (Builds repo tree, Employment Records employment-types/role-files levels) |
...
### Employment Records filter (`f`) and navigation (mouse)
...
lists every `src/content/repositories/*.md` file as its own browsable tree.
...
| entry point | mode |
|---|---|
| `:` with a Builds/Employment Records file editor open | **ex mode** — the editor's own `:q`/`:q!`/`:w`/`:wq`/`:<number>` state machine tried first (editor context always wins, e.g. `:q` closes the editor, never the pane), falling through to the site-wide command set below on anything it doesn't recognize |
| `:` anywhere else (no other text input active) | **site mode** — the site-wide command set below only; a literal `:` still types normally inside the grep query, Employment Records filter, rename prompt, and a focused shell |
...
`:builds`, `:employment`, `:profile`, `:retina-v`, `:help` (window jumps);
...
| key | action |
|---|---|
| `?` | opens the `─ Help ─` palette; empty query lists the site-wide commands (dashboard, builds, employment, profile, retina-v, help, grep, reboot, resume) |
...
- **Routes**: six static pages,
  `src/pages/{index,builds,employment,profile,retina-v,help}.astro`. Each
  ...
  `home | builds | employment | retina-v | profile | help`; the status bar's
  six windows are `0:dashboard 1:builds 2:employment 3:retina-v 4:profile
...
- **Islands**: `Terminal.svelte` owns the tmux client `$state` (sessions →
  ...
  `Dashboard` / `Builds` / `EmploymentRecords` / `Profile` / `TrackerView` /
  ...
  `EmploymentRecords` each delegate their own file-viewer state to a shared
...
- **Data flow**: `src/lib/data.ts` loads every `src/data/*.yaml` file
  ...
  themselves. Content collections (`repositories`, `personnel`) are defined in
...
- **Generated artifacts** (`scripts/generate.mjs`, wired to
  ...
  - `public/generated/repos/all-projects.json` — the same shape, built
    from `src/content/repositories/*.md` instead of a submodule checkout;
...
- **Fixture mode** (`PORTFOLIO_FIXTURES=1`, set by `pnpm build:fixtures`):
  the `repositories` content collection switches to `fixtures/repositories/*.md`
...
(tmux-faithful navigation, the Builds rework, depth-generic Employment Records, the
