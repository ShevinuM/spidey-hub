<script lang="ts">
  ...
  // Employment Records renders one of these left-aligned on its own top
  // border (mockup: UI-Mockups/builds-page-design-review/Builds.dc.html's
  // "[1] Status" pill et al; byte-identical markup repeated per panel
  // there, pulled out into one component here). Two accents: "blue" (the
  // default — red-glow/blue-border pill every Repositories/Personnel panel
  // uses) and "teal" (Help's section-header pill — Help-Panel-Changes.md /
  ...
  // verifier/test can assert on `n`/`label` independently of the bracket
  // formatting.
  //
  ...
  // underneath (the mockup's own convention, unchanged here). It sits with
  // a 12px left inset — matching the panels' own left content padding
  // (10-14px across callers) and clearing every panel's 3-4px
  // border-radius — instead of centered.
  interface Props {
    /** Panel number shown inside brackets, e.g. `0` in "[0] Status".
     * Omit together with `label` when using `left`/`right` split-text mode
     * instead (Employment Records' badges — see below). */
    n?: number;
    /** Panel label shown after the bracketed number, e.g. "Status". */
    label?: string;
    /** Split-text mode: renders `{left} {right}` as one spaced string
     * instead of `[{n}] {label}` (Employment Records' "Employment Records" /
     * "File Preview" badges, UI-Mockups Personnel.dc.html). Blue accent
  ...
  const wrapperStyle = inline
    ...
    : "position:absolute;left:0;right:0;top:0;transform:translateY(-50%);display:flex;justify-content:flex-start;padding-left:12px;pointer-events:none;z-index:4";
</script>

<div data-testid="panel-badge" data-accent={accent} style={wrapperStyle}>
  {#if accent === "teal"}
    <span
      style="display:flex;align-items:center;border:1px solid rgba(87,226,201,.5);border-radius:3px;background:rgba(5,7,10,.92);box-shadow:0 0 14px rgba(87,226,201,.16);padding:3px 12px;font:600 11.5px 'JetBrains Mono',monospace;letter-spacing:.16em;color:#57e2c9;white-space:nowrap"
    >
