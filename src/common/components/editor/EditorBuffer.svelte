...
>
  {#each state.lines as l (l.n)}
    ...
    <div data-line={l.n} style="display:flex;gap:16px;white-space:pre">
      ...
      >{#if !d}{#if typeof l.t === "string"}<span data-testid="editor-line-text" style={l.style}
            >{l.t}</span
          >{:else}<span data-testid="editor-line-text"
            >{#each l.t as [idx, text]}<span style={`color:${state.palette[idx] ?? ""}`}
                >{text}</span
              >{/each}</span
        ...
        >{:else}<span data-testid="editor-line-text" style={l.style}
          >{#each d.segments as seg}<span
              style={segStyle(seg.cls, seg.color)}
