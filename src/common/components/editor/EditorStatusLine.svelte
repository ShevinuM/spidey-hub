...
>
  <span
    ...
    style="flex:none;background:#e0453c;color:#0b0f14;font-weight:700;padding:3px 12px"
    >{state.modeOrPromptText}</span
  >
  <span style="flex:none;background:rgba(224,69,60,.22);color:#f0d9d4;padding:3px 12px"
    >{labels.branch}</span
  >
  <span
  ...
  {#if state.message}
    <span
      ...
      style="flex:none;white-space:nowrap;color:#e0453c;padding:3px 12px">{state.message}</span
    >
  {/if}
  <span style="flex:1;min-width:0"></span>
  <span
    data-testid="editor-position"
    style="flex:none;white-space:nowrap;color:#5fc6b4;padding:3px 12px"
