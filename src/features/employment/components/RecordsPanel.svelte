...
<div style="position:relative;flex:1;min-width:0;display:flex;flex-direction:column;gap:12px">
  ...
  >
    <div style="display:flex;flex-direction:column;gap:6px;padding:0 4px;font-size:11px">
      <div
        style="display:flex;align-items:center;gap:8px;color:rgba(224,69,60,.5);white-space:nowrap"
      >
      ...
      <div style="display:flex;gap:6px;white-space:nowrap;min-width:0">
        <span style="color:rgba(196,216,232,.32);flex:1;overflow:hidden;text-overflow:ellipsis"
          >{state.personnel.index.orgsLabel}</span
        >
        <span data-testid="employment-index-orgs" style="color:rgba(95,198,180,.72)"
          >{state.indexStats.orgs}</span
        >
      </div>
      <div style="display:flex;gap:6px;white-space:nowrap;min-width:0">
        <span style="color:rgba(196,216,232,.32);flex:1;overflow:hidden;text-overflow:ellipsis"
          >{state.personnel.index.longestLabel}</span
        >
      ...
      <div style="display:flex;gap:6px;white-space:nowrap;min-width:0">
        <span style="color:rgba(196,216,232,.32);flex:1;overflow:hidden;text-overflow:ellipsis"
          >{state.personnel.index.yearsActiveLabel}</span
        >
        <span data-testid="employment-index-years" style="color:rgba(217,176,74,.75)"
          >{state.indexStats.yearsStart}{state.personnel.index.yearsActiveSeparator}{state
            .indexStats.yearsEnd}</span
        >
      </div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:6px;padding:0 4px">
      <div style="display:flex;align-items:center;gap:7px;white-space:nowrap;font-size:11.5px">
        <span data-testid="employment-breadcrumb" style="color:#5fc6b4"
          >{state.personnel.breadcrumb}</span
        >
    ...
    >
      {#each state.records as row, i (row.entry.id)}
        ...
        >
          ...
          <span
            style="width:12px;height:12px;display:inline-flex;color:{on
              ? '#42a5f5'
              : 'rgba(66,165,245,.6)'}"
            aria-hidden="true">{@html state.iconFor(row)}</span
          >
          <span style="overflow:hidden;text-overflow:ellipsis">{row.name}</span>
          <span
            style="font-size:10.5px;color:{on ? 'rgba(244,236,233,.5)' : 'rgba(196,216,232,.32)'}"
            >{row.orgTag}</span
          >
          <span style="color:{on ? 'rgba(244,236,233,.7)' : 'rgba(196,216,232,.34)'}"
            >{state.personnel.filePerms}</span
          >
          <span
            style="text-align:right;color:{on ? 'rgba(244,236,233,.7)' : 'rgba(196,216,232,.34)'}"
            >{row.sizeBytes}</span
          >
        </div>
      {/each}
