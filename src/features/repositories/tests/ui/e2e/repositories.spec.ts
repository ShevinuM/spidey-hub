...
// github.com/ShevinuM/<name>, matching the `github` frontmatter each project
// doc declares), so panel [1]'s flat repo list is those 8 rows PLUS the virtual "all-projects"
...
// (`pnpm generate`, wired to prebuild) from SHA-pinned GitHub tarballs and
...
/** Reads a real file straight off disk — from the SHA-pinned tarball cache
 * scripts/generate.mjs populates at `.cache/repos/<name>-<sha>/`, not from
 * `public/generated/repos/<name>.json` — so assertions below are checked
 * against ground truth independent of generate.mjs's own read/tokenize
 * path, not a second copy of the same transformed string. `<sha>` is the
 * same pin generate.mjs itself resolves, read from the committed
 * repos.json. */
