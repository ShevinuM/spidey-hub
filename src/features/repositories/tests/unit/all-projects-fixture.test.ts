...
// `src/features/repositories/tests/ui/support/repos/all-projects.json` is HAND-COMMITTED (not
// regenerated by `pnpm generate`, which only ever touches
// `public/generated/repos/all-projects.json` from the REAL
// `src/features/repositories/content/repositories/*.md` — see scripts/generate.mjs's
...
// sync with `src/features/repositories/tests/ui/support/repositories/*.md` — nothing enforces that automatically
...
// from `src/features/repositories/tests/ui/support/repositories/*.md`, mirroring `generateAllProjectsIndex()`'s
// exact logic (one entry per `.md` file, `path` = filename verbatim, `lines`
// = the raw file text split on "\n", sorted by `path.localeCompare`), and
// asserts it deep-equals the committed `src/features/repositories/tests/ui/support/repos/all-projects.json`
 ...
 * substituting `src/features/repositories/tests/ui/support/repositories` for `src/features/repositories/content/repositories` — same
