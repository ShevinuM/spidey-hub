...
<style>
  ...
  /* Forcing `animation: none` while the boot overlay is present, rather than pausing, means the resolved `animation-name` flips from `none` to a real value in one style recalc when the overlay unmounts — a value change the spec treats as a brand-new animation instance — so `toastIn`, `strand`, and `drain` all start together, fresh, the instant a toast becomes visible. */
