...
/** PLAN.md Iteration 3 Phase 6 item 6.1 — a real tmux split: N children laid
...
export interface Window {
  ...
  activePaneId: string;
  /** PLAN.md Iteration 3 Phase 6 item 6.2 — prefix `;` (last-pane), the
   * pane-focus twin of `Session.lastWindowIdx`. The pane that was focused
   * immediately before the CURRENT one; bookkept by every op that reassigns
   * `activePaneId` below. Undefined until the window's focus has actually
   * moved at least once (fresh window: nothing to jump back to). */
  lastPaneId?: string;
  /** PLAN.md Iteration 3 Phase 6 items 6.1/6.4 — every pane id currently in
   * this window's tree, in CREATION order: index 0 is the "main" pane for
   * `main-horizontal`/`main-vertical` layouts (documented tmux fidelity
   * choice — real tmux uses the first-created pane the same way), and a
   * pane's live position in this array is its `pane_index` for the
   * `kill-pane {pane_index}? (y/n)` prompt (PLAN.md Locked decision #4).
   * Deliberately RENUMBERED on every kill (indices always `0..n-1`, no
   * gaps) rather than preserving each pane's original creation number —
   * documented simplification (PLAN.md leaves the exact numbering scheme to
   * the executor); layout application never reorders this array, only
   * split/kill do. */
  paneOrder: string[];
  /** PLAN.md Iteration 3 Phase 6 item 6.1 — a monotonic per-window counter
   * (never reused, never derived from `paneOrder.length`) backing every new
   * pane id this window ever creates (`${windowId}#${paneSeq++}`), so a
   * kill-then-split sequence can never mint an id that collides with a pane
   * still alive in the tree (advisor-caught: deriving the next index from
   * `paneOrder.length` would do exactly that). Starts at 1 — index 0 is
   * always the window's original factory-seeded pane. */
  paneSeq: number;
  /** PLAN.md Iteration 3 Phase 6 item 6.4 — the last preset applied via
   * `Ctrl-b Space`/`select-layout` (undefined = "never applied one yet", the
   * fidelity reference's "lastLayout = -1" — the next bare Space starts the
   * cycle at index 0, `even-horizontal`). A manual split does NOT reset
   * this — real tmux only tracks the last APPLIED preset, a hand split just
   * makes the window's actual geometry diverge from it until Space (or
   * `select-layout`) is used again. */
 ...
 * array order) — Phase 4: exactly one; Phase 6: every leaf under a split. */
...
/** The window that owns `paneId`, wherever it is in the session (PLAN.md
 * Iteration 3 Phase 6 — `onExitPane`/kill-pane call sites need to resolve an
 * ARBITRARY pane's owning window, not just the currently-active one, now
 * that a session can have more than one pane per window). */
...
// ---------------------------------------------------------------------------
// Pane-tree rewrite helpers (PLAN.md Iteration 3 Phase 6 item 6.1) — `split
// Pane`/`killPaneInWindow` below each walk the tree by paneId (never by
// object identity) and rebuild new objects back up to the root, structural-
// sharing every untouched subtree — see `insertSplit`'s own comment
// (further down, right beside `splitPane`) for the split-side rebuild.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Pane focus / navigation (PLAN.md Iteration 3 Phase 6 item 6.2 — prefix
// `o`/arrows/`;`). Every one of these bookkeeps `lastPaneId` the same way
 ...
 * ever needs relative position, and this is exactly the fraction PaneTree.
 * svelte's flex rendering itself uses for `sizes` (PLAN.md Phase 6 item
 * 6.1/6.2 share this one geometry model). */
...
/** Geometric directional pick (PLAN.md 6.2 "pick the nearest pane in that
 * direction from the focused pane's rect"): among every OTHER pane whose
...
// ---------------------------------------------------------------------------
// Splits (PLAN.md Iteration 3 Phase 6 item 6.1 — `Ctrl-b |`/`%` and `-`/`"`)
// ---------------------------------------------------------------------------
 ...
 * SHELL pane (PLAN.md 6.1 "new panes run shell"), which becomes the newly
 * focused pane. A no-op if the window's `activePaneId` has somehow gone
...
// ---------------------------------------------------------------------------
// Kill-pane (PLAN.md Iteration 3 Phase 6 item 6.3 / Locked decision #4)
// ---------------------------------------------------------------------------
...
/** Removes `paneId` from `window`'s tree (PLAN.md Locked decision #4: `Ctrl-b
 * x` ALWAYS prompts kill-pane, even on a single-pane window — the caller is
 * the one that decides what "the last pane" cascades to, this function only
...
// ---------------------------------------------------------------------------
// Layout engine (PLAN.md Iteration 3 Phase 6 item 6.4 — the 7 tmux presets
// and `Ctrl-b Space` / `select-layout`)
// ---------------------------------------------------------------------------
 ...
 * `main-vertical(-mirrored)` (PLAN.md tmux fidelity reference: "big main
 * pane" + others along the opposite edge). Real tmux sizes its main pane
 * with an ABSOLUTE `main-pane-height`/`main-pane-width` (a fixed row/column
 * count), not a percentage, so there is no single "exact" fraction to port —
 * 60/40 is a deliberate, documented choice (advisor-confirmed) that keeps
 * the main pane visibly bigger than the rest, satisfying every bounding-box
 * assertion the fidelity reference implies without claiming false
 * precision. */
 ...
 * continues the cycle/reapplication from here (PLAN.md 6.4: the named form
 * and the cycle share one piece of state). A no-op if the window somehow has
 ...
 * no-op if no preset has ever been applied to this window (PLAN.md 6.4). */
 ...
 * `killSession` (choose-tree `x` on a session row / PLAN.md Iteration 3
 * Phase 6 item 6.5) — the exact same "session is gone" bookkeeping either
 * way, just reached from two different triggers. */
...
/** choose-tree `x` on a SESSION row (PLAN.md Iteration 3 Phase 6 item 6.5) —
 * kills every window in `sessionId` at once, i.e. the whole session, in one
 * step (unlike `killWindowCascade`, which only ever destroys a session as a
 * SIDE EFFECT of its last window dying). Locked decision #16 only excludes
 * the TYPED `kill-session` cmdline/tmux command, not this overlay action.
 * Reuses the exact same "destroy a session" bookkeeping `killWindowCascade`
 * falls back to. */
