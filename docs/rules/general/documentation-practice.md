...
How this project's own documentation (including these rules) is written and maintained.
...
- [ ] **R003** Docs are updated in the same change that invalidates them — a decision lands with its rules entry; a mechanism change sweeps every doc that described the old mechanism.
- [ ] **R004** A new ruling is added to the relevant rules file as a short, prescriptive entry at the moment it's decided — not batched for later.
- [ ] **R005** A code change that alters a canonical example file (one this rules doc or another doc points at) updates the doc's pointer in the same change.
