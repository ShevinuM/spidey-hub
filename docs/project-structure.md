...
| `personnel` | one role per file, depth-generic path (backs Employment Records) | yes — `fixtures/personnel/*` under `PORTFOLIO_FIXTURES=1` (mirrors the real tree's variable-depth shape, not just its record count) |
