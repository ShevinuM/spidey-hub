...
- [ ] **R002** Shared fixtures, harnesses, and test-only helpers for the UI suites live in a `tests/ui/support/` folder at the level they're shared from (a feature's own `tests/ui/support/` if only that feature needs them, `common/tests/ui/support/` if more than one does) — never duplicated per consumer. This also covers shared visual-suite machinery (recipes, capture pipeline, static server, capture script, the fixture-flag check, a vendored reference asset tree) once more than one context replays it — same folder, not a one-off script living elsewhere. `common`'s own test homes (`common/tests/ui/{e2e,visual,support}/`, `common/tests/unit/`) sit at the top level, a sibling of `src/`, not nested under `src/common/` — `src/common/` holds only application source (`engines/`, `components/`, `lib/`, `content/`), matching `architecture.md` R001's skeleton, which lists no `tests/` under it.
