...
## Feature harness specs

- [ ] **R018** A harness spec (`../testing/README.md` R011) asserts only what the standalone component itself can prove without a kernel present: mount + content + the component's own self-contained behavior (a timer loop, an internal state transition). It never asserts a keyboard shortcut/hotkey that a real page only wires up through the kernel's own keydown delegation (`Terminal.svelte`) — that delegation doesn't exist in the harness, so the assertion would either no-op or need reimplementing the delegation just for the test. It also asserts no goldens (`../testing/README.md` R011's "no goldens for now") and, where relevant, that no kernel chrome (status bar, window switching) leaked into the mount.
