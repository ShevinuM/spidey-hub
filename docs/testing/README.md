...
| `pnpm lint`         | `oxlint`                                                             | oxlint's default rule set over the repo's `.ts`, `.js` and `.mjs` files, the `<script>` block of every `.svelte` file, and the frontmatter of every `.astro` file. `.oxlintrc.json` sets ignore patterns only. Findings print as warnings and the command still exits 0, so this one reports rather than gates.                                                                                              |
