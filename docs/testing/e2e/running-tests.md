...
| `pnpm test:visual` | Fixture build then the shared/legacy visual specs plus every split-out `<context>-visual-<viewport>` project (see `../visual/running-tests.md`) plus the `profile-harness`, `help-harness`, `boot-harness`, and `notifications-harness` functional (non-golden) projects. |
