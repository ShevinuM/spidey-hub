...
- **Verify**: visual `09/10` pass both viewports (fixture index reproduces the prototype's 24-file snapshot: identical rows, counts `24/24` empty and `N/M` for "svelte", identical preview); e2e on real index: `/` from profile opens overlay, typing filters (a known real path e.g. `src/lib/grep.ts` findable by its own name), Enter on a personnel content hit lands in personnel view, Esc closes back to prior view, Ctrl-u clears, counter format `X/Y` correct against the real index JSON.
  - Index-loading mechanism: single canonical fetch path (`/generated/grep-index.json`) in both builds; `pnpm build:fixtures` overwrites the built copy with the fixture's 24-file snapshot as its very last step (`cp fixtures/grep-index.json dist/generated/grep-index.json` in package.json), so the exact same JS bundle runs in goldens and production — only the JSON payload differs. `astro dev` with `PORTFOLIO_FIXTURES=1` does not serve the fixture index (unsupported; no workflow needs it).
